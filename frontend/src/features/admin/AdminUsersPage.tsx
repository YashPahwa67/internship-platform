...
export default function AdminUsersPage() {
  ...
  } = useGetUsersQuery({}, { refetchOnMountOrArgChange: true, refetchOnFocus: true });

  const {
    data: deletedData,
    refetch: refetchDeleted,
    isFetching: fetchingDeleted,
  } = useGetUsersQuery({ status: 'deleted' }, { refetchOnMountOrArgChange: true, refetchOnFocus: true });

  const isFetching = fetchingActive || fetchingDeleted;

  const handleRefresh = () => { refetchActive(); refetchDeleted(); };
  ...
  const handleDelete = async (u: User) => {
    if (!window.confirm(`Delete account for ${name(u)}?\nThey will lose access immediately and appear in the Deleted Accounts tab.`)) return;
  ...
  const handleRestore = async (u: User) => {
    if (!window.confirm(`Restore account for ${name(u)}?\nTheir status will be set back to active.`)) return;
  ...
  return (
    <Box>
      ...
      <Tabs
        ...
        sx={{ mb: 3, borderBottom: `1px solid ${t.border}` }}
      >
        <Tab
          label={
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              {activeUsers.length > 0 && (
                <Chip label={activeUsers.length} size="small" sx={{ height: 20, fontSize: '0.72rem', fontWeight: 600 }} />
              )}
            </Box>
          }
        />
          ...
          label={
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              {deletedUsers.length > 0 && (
                <Chip
                  label={deletedUsers.length}
                  size="small"
                  sx={{ height: 20, fontSize: '0.72rem', fontWeight: 700, bgcolor: alpha('#ef4444', 0.12), color: '#ef4444' }}
                />
              )}
            </Box>
          }
            ...
            <FadeIn key={u.id} delay={(i % 3) as 0 | 1 | 2}>
              <PremiumCard hover={false} sx={{ mb: 1.5 }}>
                <CardContent sx={{
                  ...
                  <Box sx={{ minWidth: 0 }}>
                    <Typography fontWeight={600} sx={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {name(u)}
                    </Typography>
          ...
          {deletedUsers.length === 0 && (
            <Box sx={{ textAlign: 'center', py: 12 }}>
              ...
              <Typography variant="body2" color="text.disabled">
                Accounts you delete will appear here and can be restored at any time.
              </Typography>
            </Box>
          )}
                ...
                <CardContent sx={{
                  ...
                  <Box sx={{ minWidth: 0 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.25, flexWrap: 'wrap' }}>
                      <Typography
                        fontWeight={600}
                        sx={{ textDecoration: 'line-through', color: 'text.secondary', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                      >
                        {name(u)}
                      </Typography>
                      <Chip
                        label="Deleted"
                        size="small"
                        sx={{ height: 20, fontSize: '0.7rem', fontWeight: 700, bgcolor: alpha('#ef4444', 0.1), color: '#ef4444', flexShrink: 0 }}
                      />
                    </Box>
                    ...
                    <Box sx={{ display: 'flex', gap: 2.5, mt: 0.5, flexWrap: 'wrap' }}>
                      ...
                      {u.deletedAt && (
                        <Typography variant="caption" sx={{ color: alpha('#ef4444', 0.65) }}>
                  ...
                  <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', flexShrink: 0 }}>
                    ...
                    <Button
                      ...
                      sx={{
                        borderColor: alpha('#22c55e', 0.4), color: '#22c55e',
                        '&:hover': { borderColor: '#22c55e', bgcolor: alpha('#22c55e', 0.07) },
                      }}
                    >
                      Restore
                    </Button>
